// Implementation for ACMOJ 1779 - Linked Hash Map
...
public:
    ...
    // function pointer for traversal method
    ...
    ~LinkedHashMap(){
        // delete all nodes in buckets to free memory
        for(int i=0;i<len;i++){
            Node* p=array[i];
            while(p){
                Node* nxt=p->next;
        ...
        delete [] array;
        delete beg;
        // current points into freed nodes; no need to delete
    }
...
inline vector<Data> traverseByTime(const LinkedHashMap& obj){
    vector<Data> res;
    // traverse from first real element after beg via timeNext
